	...
	<div class="main">
		<header class="query-nav">
			<div class="search-input">
				...
				<input type="search" id="search" placeholder="search for a country" @keyup.enter="getCountry($event)"
					v-model.lazy="searchQuery" />
			</div>
			<label for="select-region" class="sr-only">select region</label>
			<select name="Filter By Region" id="select-region" v-model="regionSelected">
				<option disabled selected hidden value="">Filter by region</option>
				<option v-for="region in Regions" :key="region" tabindex="0">
					{{ region }}
				</option>
			</select>
		</header>
